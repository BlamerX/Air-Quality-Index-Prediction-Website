...
def predict():
    ...
    if 0 <= prediction <= 50:
        ...
        return render_template('result1.html', aqi=prediction, aqi_range=aqi_range, text=text, health_impact=health_impact)
    elif 51 <= prediction <= 100:
        text = "Satisfactory"
        health_impact = "Minor breathing discomfort to sensitive people"
        # Return the predicted AQI, color code, and health impact as a response
        return render_template('result2.html', aqi=prediction, aqi_range=aqi_range, text=text, health_impact=health_impact)
    elif 101 <= prediction <= 200:
        text = "Moderate"
        health_impact = "Breathing discomfort to people with lungs, asthma, and heart diseases"
        # Return the predicted AQI, color code, and health impact as a response
        return render_template('result3.html', aqi=prediction, aqi_range=aqi_range, text=text, health_impact=health_impact)
    elif 201 <= prediction <= 300:
        text = "Poor"
        health_impact = "Breathing discomfort to most people on prolonged exposure"
        # Return the predicted AQI, color code, and health impact as a response
        return render_template('result4.html', aqi=prediction, aqi_range=aqi_range, text=text, health_impact=health_impact)
    elif 301 <= prediction <= 400:
        text = "Very Poor"
        health_impact = "Respiratory illness on prolonged exposure"
        # Return the predicted AQI, color code, and health impact as a response
        return render_template('result5.html', aqi=prediction, aqi_range=aqi_range, text=text, health_impact=health_impact)
    elif 401 <= prediction <= 500:
        text = "Severe"
        health_impact = "Affects healthy people and seriously impacts those with existing diseases"
        # Return the predicted AQI, color code, and health impact as a response
        return render_template('result6.html', aqi=prediction, aqi_range=aqi_range, text=text, health_impact=health_impact)
